...
public class AccountController {
    ...
    @ResponseBody
    public TableData<Account> find(Integer page, Integer limit, Account account) {
        PageInfo<Account> pageInfo = accountService.findPage(page, limit, account);
        TableData<Account> tableData = new TableData<>("ok", 0, pageInfo.getList(), pageInfo.getTotal());
        return tableData;
    }

    @RequestMapping("/batDel")
    @ResponseBody
    public int batDel(@RequestParam List<Integer> ids) {
        int count = 0;
